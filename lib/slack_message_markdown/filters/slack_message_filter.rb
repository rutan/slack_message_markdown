  ...
  module Filters
    class SlackMessageFilter < HTML::Pipeline::TextFilter
      def call
        ...
        link_list.map { |n| "&lt;#{n}&gt;" }.each do |pattern|
          html.gsub!(pattern) do |match|
            case match[4]
            when '@'; link_mention(match[5, match.size - 9])
            when '#'; link_channel(match[5, match.size - 9])
      ...
      def link_mention(uid)
        user = self.context[:on_get_user] ? self.context[:on_get_user].call(uid) : nil
        if user
      ...
      def link_channel(uid)
        channel = self.context[:on_get_channel] ? self.context[:on_get_channel].call(uid) : nil
        if channel
